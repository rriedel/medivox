//! Nimmt Audio zwischen start() und stop() auf (unveraendert uebernommen von
//! ../shim-rust/src/recorder.rs). Kein VAD -- Start/Stop ist explizit.
//!
//! cpal spricht auf macOS CoreAudio statt WASAPI, die API ist identisch. Der Downmix
//! auf Mono passiert direkt im Capture-Callback (haelt den Puffer klein), das Resampling
//! auf 16 kHz einmalig beim Stop.
...
impl Recorder {
    ...
    pub fn stop(&mut self) -> Result<Vec<f32>> {
        ...
        let resampled = self.finalize_chunk(samples)?;
        tracing::debug!(
    ...
    /// Liefert seit dem letzten Aufruf neu aufgenommene Samples als 16 kHz mono float32.
    pub fn drain_chunk(&mut self) -> Result<Vec<f32>> {
        ...
        self.finalize_chunk(samples)
    }

    fn finalize_chunk(&self, samples: Vec<f32>) -> Result<Vec<f32>> {
        if samples.is_empty() {
            return Ok(Vec::new());
        }
        ...
        resample(&samples, self.source_rate, SAMPLE_RATE)
    }
}
